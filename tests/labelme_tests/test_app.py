...
def test_MainWindow_open(qtbot):
    win = labelme.widgets.app.MainWindow()
    qtbot.addWidget(win)
    win.show()
    win.close()
...
def test_MainWindow_open_img(qtbot):
    ...
    win = labelme.widgets.app.MainWindow(filename=img_file)
    qtbot.addWidget(win)
...
def test_MainWindow_open_json(qtbot):
    ...
    for json_file in json_files:
        labelme.testing.testing.assert_labelfile_sanity(json_file)

        win = labelme.widgets.app.MainWindow(filename=json_file)
        qtbot.addWidget(win)
        _win_show_and_wait_imageData(qtbot, win)
        win.close()
...
def test_MainWindow_open_dir(qtbot):
    ...
    win = labelme.widgets.app.MainWindow(filename=directory)
    qtbot.addWidget(win)
    _win_show_and_wait_imageData(qtbot, win)
    return win
...
def test_MainWindow_annotate_jpg(qtbot):
    ...
    labelme.testing.testing.assert_labelfile_sanity(out_file)
